...
class FlipPredictor(object):
    ...
    def pheads(self):
        return sum(coin * prob for coin, prob in zip(self.coins, self.probs))
    ...
    def update(self,result):
        ...
        for i in range(len(self.probs)):
            if result == 'H':
                pChange = self.coins[i] / pheads
            else:
                pChange = (1 - self.coins[i]) / (1 - pheads)

            self.probs[i] = self.probs[i] * pChange
